...
function WaterForm() {
  ...
  function calculateTotal() {
    ...
    for (let i = 0; i < filteredEntries.length; i++) {
      totalAmount += filteredEntries[i].amount;
    }

    if (totalAmount > 0) {
      setTotal(`${totalAmount} ml`);
    } else {
      setTotal("No entries found!");
    }
  }
    ...
    <div className="container my-4 p-4 border rounded shadow-sm">
      ...
      <div className="mb-4">
        ...
        {visibleEntries.length === 0 ? (
          <div className="alert alert-info">
            No entries found for your account.
          </div>
        ) : (
                ...
                >
                  {editingIndex === realIndex ? (
                    ...
                    <div className="w-100 d-flex justify-content-between align-items-center">
                      <span>
                        <span className="fw-bold">{e.date}</span> — {e.amount}ml
                        at <small className="text-muted">{e.time}</small>
                      </span>
                      <div>
                        <button
          ...
          <ul className="pagination">
            ...
            <li className="page-item disabled">
              <span className="page-link">
                Page {page} of {Math.ceil(userEntries.length / itemsPerPage)}
              </span>
            </li>
            <li
              className={`page-item ${
                end >= userEntries.length ? "disabled" : ""
              }`}
            >
      ...
      <div className="p-3 border rounded bg-light">
        ...
        {total && (
          <p
            className={`mt-3 mb-0 ${
              total.includes("No entries") || total.includes("Please select")
                ? "text-danger"
